...
const TOUR_V = 2;
/* Короткий тур («где что», три вкладки) — строка «Подсказки» на «Сегодня»; полный набор шагов остается на будущее (строка в аккаунте снята 20.09). */
const TOUR_STEPS = [
  ...
  { target: '#h-acct', text: 'Аккаунт. Здесь настраивается ваш ежедневный ритм: три уведомления — утром настрой дня, вечером «запомнить день», раз в неделю история про вас. Рядом шестеренка — светлая или темная тема.' },
...
function paintTourRow(){
  ...
  box.innerHTML = due ? row('Подсказки', ui('home.tips_row', 'Как устроено приложение'), 'За 20 секунд →', 'click:tourShort', 'id="tour-row"') : '';
}

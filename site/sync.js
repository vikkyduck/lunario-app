/* Черновик не теряется: сначала на устройство, потом на сервер.
   ...
   Зачем. Сейчас несохранённая запись живёт только в поле ввода. Перезагрузка, уснувший телефон, оборванная
   связь — и текста нет. Здесь он сначала ложится в хранилище браузера и только потом уходит на сервер;
   не ушёл — полежит и уйдёт позже, под тем же именем операции, поэтому вторая запись не появится.

   Три правила, без которых всё это не имеет смысла:
   · «Сохранено на устройстве» говорим только после того, как браузер подтвердил запись (complete у транзакции),
     а не когда мы её попросили. Иначе обещание ложное, и человек спокойно закроет вкладку.
...
(function () {
  ...
  async function sendOne(op) {
    ...
    /* попытку считаем ДО обращения к сети: иначе оборванный ответ не увеличил бы счётчик и повторы шли бы вечно */
    ...
    try {
      ...
      notice(
        state === 'conflict' ? 'Данные требуют проверки. Черновик сохранён на устройстве.'
          : err.status === 401 ? 'Войдите снова. Черновик сохранён на устройстве.'
          : err.status === 403 ? 'Нет доступа к этому действию. Ваш текст сохранён на устройстве.'
          : state === 'queued' ? 'Связь нестабильна. Черновик сохранён на устройстве; отправим, когда появится связь.'
          : 'Отправка приостановлена. Черновик сохранён на устройстве.');
      return next;
    }
  }
  ...
  async function saveJournal(text, accountId, kind = '') {
    ...
    notice('Черновик сохранён на устройстве.');
  ...
  const LUN_SYNC = {
    ...
    saveJournal,
    /* вызвать при старте и при возвращении связи: разберёт, что осталось в очереди этого человека */
    resume: (accountId) => serial(() => flush(accountId)),
    /* сколько операций ждёт отправки — для честного статуса на экране */
    async pending(accountId) {
      const rows = await run(OPS, 'readonly', (s) => s.getAll());
      return rows.filter((r) => r.owner === accountId && r.state !== 'synced').length;
    },
  };
  /* Связь вернулась — пробуем снова. Сам по себе этот сигнал доступности сервера не доказывает: отправка всё
     равно идёт через обычные повторы с задержкой. */

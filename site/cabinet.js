...
const EXTRAS = {
  ...
  async backlog() {
    ...
    $('extra').innerHTML = `${r.canCreate ? `<div class="row" style="justify-content:flex-end;margin-top:14px"><button data-on="click:taskForm-0" class="btn gold sm fixed">+ Новая задача</button></div>` : `<div class="notice">Вы видите задачи, поставленные вашей роли${r.own ? ` («${esc(r.roles[r.own])}»)` : ''}. Меняйте статус — постановщик увидит его сразу.</div>`}

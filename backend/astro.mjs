...
/* Луна: ряды Меёса, таблицы 47.A / 47.B */
...
function placidus(ramc, phi, eps) {
  ...
  const cusp = (offset, frac, nocturnal) => {
    ...
    for (let i = 0; i < 60; i++) {
      ...
      if (Math.abs(x) > 1) return null;                              // за полярным кругом Плацидус не определён
...
export function natalChart({ birth, time, tzOffsetMin = 0, lat = null, lon = null }) {
  ...
  if (timeKnown && hasPlace) {
    const ramc = norm(gmst(JD) + lon);                       // местное звёздное время = RA MC

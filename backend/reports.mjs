/* Отчеты рабочих кабинетов. Один источник данных — те же таблицы, что у приложения.
   Каждый отчет отдает одинаковую структуру: kpis → charts → tables → how.
   У показателя три честных состояния: число, «нет данных» (событие еще не собирается)
...
export const REPORT_META = {
  ...
  acquisition:   ['Привлечение и кампании', 'Откуда приходят люди и сколько стоит тот, кто остается'],
  campaigns:     ['Кампании и UTM-ссылки', 'Что запускаем, сколько стоит и какой ссылкой ведем'],
  ...
  concerns:      ['Темы и интересы', 'О чем спрашивают и какие ответы выбирают'],
  ...
  saved:         ['Сохраненные отчеты', 'Отчеты с фильтрами, сохраненные в этом браузере'],
...
/* Функции продукта и содержательные действия — из общего реестра events.mjs (один источник для сервера и отчетов) */
...
/* Кэш на время одного отчета. Когорты перебираются в циклах (человек × окно), и раньше на каждого уходил
   свой запрос; теперь активные дни, первое действие, отметки настроения и подписки читаются одним запросом
   на всех и дальше ищутся в памяти. Живет только внутри overview()/report(): следующий отчет видит свежие данные. */
let memo = null;
...
function levels(to) {
  ...
  const lv = { 'Ядро': 0, 'Регулярные': 0, 'Редкие': 0, 'Спящие': 0, 'Предупрежденные': 0, 'К удалению': 0 };
  for (const u of users) {
    ...
    else if (gone >= 61) lv['К удалению']++; else if (gone >= 31) lv['Предупрежденные']++; else if (gone >= 15) lv['Спящие']++;
  }
  return lv;
}
...
export const overview = withMemo(function overview(q) {
  ...
  const charts = [
    ...
    chart('bars', 'Использование разделов', 'уникальные пользователи за период', sectionReach(P.from, P.to), 'Сравнивайте охват вместе с повторным использованием в отчете функций.'),
  ];
  ...
  const allBlocks = [
      { key: 'new_users', to: 'acquisition', title: 'Новые с подтвержденной почтой', value: nu, prev: nuP, delta: delta(nu, nuP), unit: 'чел.', series: nuSeries, sub: 'завершили регистрацию за период' },
      ...
      { key: 'problems', to: 'system', title: 'Требуют внимания', value: pr.total, prev: prP.total, delta: delta(pr.total, prP.total), unit: 'сигналов', sub: pr.items.filter((i) => i.value > 0).map((i) => i.name).join(', ') || 'все спокойно', alarm: pr.total > 0, good: 'down' },
  ];
  return {
    charts,
...
function problems(from, to) {
  ...
  const items = [
    { key: 'login', name: 'Ошибки входа', value: lc.wrong + lc.expired, hint: `неверный код 3+ раза — ${lc.wrong}, код не введен до истечения — ${lc.expired}` },
...
const builders = {
  acquisition(R, { P, days, q, today }) {
    ...
    R.tables = [
      table('По источникам', ['Источник', 'Регистрации', 'Активированы', 'D7', '4-я неделя'],
        ...
        'Источник — utm_source из ссылки, которой человек пришел впервые; без UTM — приглашение или прямой заход.'),
  ...
  funnel(R, { P }) {
    ...
    R.kpis = [
      ...
      nodata('Среднее время шага', 'время на каждом поле анкеты еще не фиксируется'),
    ];
    ...
    R.tables = [
      table('Две ветки входа', ['Ветка', 'Людей', 'Что дальше'], [['Заполнить анкету', steps[1][1], 'анкета → почта → код → главная → первый результат'], ['Посмотреть без анкеты', 'нет данных', 'заглушка «В разработке» — событие еще не собирается'], ['Гости, позже заполнившие анкету', 'нет данных', 'появится вместе с веткой без анкеты']]),
      table('Остановки на полях анкеты', ['Поле', 'Остановились'], [], 'Событие остановки на конкретном поле еще не собирается — только начало и завершение анкеты.'),
    ];
  ...
  delivery(R, { P }) {
    ...
    R.how = 'Запрос — событие отправки кода. Подтверждение — верно введенный код. Неверный — код с 3 и более ошибочными попытками; просроченный — не введен за 15 минут.';
  },
  viral(R, { P }) {
    ...
    R.kpis = [
      ...
      kpi('Зарегистрировались приглашенные', invitedReg, { unit: 'чел.' }),
      kpi('Новых от 1 активного за месяц', kFactor, { unit: '', sub: 'приглашенные регистрации / MAU' }),
    ];
    ...
    R.tables = [
      table('Приглашенные и органика', ['Группа', 'Регистрации', 'D7'], [['Приглашенные', inv.length, d7(inv)], ['Органика', org.length, d7(org)]]),
    ...
    R.how = 'Приглашенный — аккаунт с полем «кто пригласил», заполненным при первом входе по ссылке. Виральность — приглашенные регистрации за 30 дней на одного активного за тот же срок.';
  },
  audience(R, { P }) {
    const plat = userPlatforms();
    ...
    R.how = 'Портрет — все аккаунты с подтвержденной почтой на сегодня, без фильтра периода. Платформа — по User-Agent последней сессии.';
  },
  concerns(R, { P }) {
  ...
  feedback(R) {
    R.kpis = [off('NPS', 'вопрос внутри приложения еще не задается'), off('«Было полезно?»', 'точка сбора после результата не запущена'), off('Причины удаления', 'необязательный вопрос при удалении не запущен'), nodata('Ответивших', '—')];
    ...
    R.how = 'NPS задается одним вопросом не чаще раза в квартал. Оценка «было полезно» отражает впечатление, а не точность прогноза.';
  },
  activity(R, { P, days }) {
    const dau = activeUsers(P.to, P.to, FUNC), wau = activeUsers(addDays(P.to, -6), P.to, FUNC), mau = activeUsers(addDays(P.to, -29), P.to, FUNC);
  ...
  retention(R, { P, today }) {
    ...
    R.tables = [table('Когорты по неделе регистрации', ['Неделя', 'Когорта', 'D1 %', 'D7 %', '4-я нед. %', 'D30 %'], weeks, 'Прочерк — когорта еще не дожила до этого дня. Это не ноль.'),
  ...
  cohorts(R, { P, today }) {
    ...
    const groups = { 'Источник': (u) => sourceOf(u), 'Платформа': (u) => plat[u.id] || 'Неизвестно', 'Первая функция': (u) => { const f = firstFunc(u.id); return f ? FNAME[f.type] || f.type : 'без функций'; }, 'Приглашение': (u) => (u.invited_by ? 'Приглашенные' : 'Остальные'), 'Напоминания': (u) => (hasPush(u.id) ? 'С напоминаниями' : 'Без') };
    const rows = [];
    ...
    R.kpis = [kpi('Людей в сравнении', users.length, { unit: 'чел.', sub: 'все с подтвержденной почтой до конца периода' })];
    R.tables = [table('Группы', ['Разрез', 'Группа', 'Людей', 'Активированы', 'D7', 'D30'], rows, 'Прочерк — группа еще не дожила до окна. Малые группы читайте с осторожностью.')];
  ...
  topics(R, { P }) {
    ...
    R.kpis = [
      ...
      kpi('«Показывать все»', chose.length + allOn.length ? pct(allOn.length, users.length || 1) : null, { unit: '% настроивших', sub: `${allOn.length} чел. читают все разделы` }),
    ...
    R.tables = [
      table('Темы', ['Тема', 'В выборе (чел.)', 'Разворачивали скрытой (раз)', 'Разворачивали, но не выбрали'], keys.map((k) => [labels[k], count[k] || 0, expand[k] || 0, expand[k] && !count[k] ? 'да — кандидат в набор по умолчанию' : '']), 'Разворот — открытие скрытого раздела по «Показать все» или по его заголовку.'),
  ...
  rituals(R, { P, days }) {
    ...
    const st = steady(P.to);
    /* «own:…» — настроение, названное своим словом: это личный текст. В отчет идет только счетчик,
       само слово не показывается — то же правило, что и в аналитике (server.mjs, mood_set). */
    const moodDist = all("SELECT CASE WHEN mood LIKE 'own:%' THEN 'свое слово' ELSE mood END mood, COUNT(*) n FROM moods WHERE day BETWEEN ? AND ? GROUP BY 1 ORDER BY n DESC", P.from, P.to);
    ...
    R.tables = [table('Механики удержания', ['Механика', 'Показатель', 'Значение'], [
      ...
      ['Недельный ИИ-отчет', 'создано / открыто', 'не запущено'], ['Привычки и благодарности', 'продолжение на 2-й и 4-й неделе', 'не запущено'], ['Вопрос дня, число дня, аффирмация', 'повторные открытия', 'нет данных — событие не собирается'],
    ])];
  ...
  ai(R) {
    R.kpis = [off('Расходы на ИИ сегодня', 'ИИ-модели в приложении не подключены'), off('Токены за месяц', ''), off('Стоимость открытого отчета', ''), off('Невостребованные материалы', '')];
    R.tables = [table('Журнал ИИ-вызовов', ['Время', 'Пользователь (id)', 'Функция', 'Запуск', 'Поставщик', 'Модель', 'Версия инструкции', 'Вход', 'Выход', 'Кэш', 'Стоимость', 'Задержка', 'Статус', 'Создан', 'Показан', 'Открыт'], [], 'Структура журнала готова; записи появятся с первым подключенным сценарием. Кэшированные токены входят во входные и не прибавляются второй раз. Тексты запросов и ответов в кабинете не показываются.')];
    R.notes = ['Повторное открытие сохраненного результата — просмотр, не новый расход.'];
  ...
  lifecycle(R, { today }) {
    ...
    R.kpis = [kpi('Аккаунтов сейчас', withMail, { unit: 'с почтой', sub: `всего записей в живой базе — ${total}` }), kpi('Зарегистрировались за все время', everRegistered, { unit: '', sub: 'накопительный счетчик, не уменьшается' }), kpi('Спящие', lv['Спящие'], { unit: 'чел.', sub: '15–30 дней' }), kpi('Удалятся в ближайшие 30 дней', del30, { unit: 'чел.', sub: 'если автоудаление включат', good: 'down' }), off('Предупреждено за месяц', 'письмо о бездействии не отправляется'), off('Удалено автоматически', 'автоудаление не запущено')];
    R.charts = [chart('funnel', 'Стадии', 'по сроку отсутствия', [['Активные (≤14 дней)', lv['Ядро'] + lv['Регулярные'] + lv['Редкие']], ['Спящие 15–30', lv['Спящие']], ['Предупрежденные 31–60', lv['Предупрежденные']], ['К удалению 61+', lv['К удалению']]])];
    R.tables = [table('События жизненного цикла', ['Событие', 'За месяц'], [['Предупреждение отправлено', 'не запущено'], ['Предупреждение не доставлено', 'не запущено'], ['Вход после предупреждения', 'не запущено'], ['Удалено автоматически', 'не запущено'], ['Удалено самостоятельно', 'нет данных — событие удаления не собирается'], ['Повторная регистрация после удаления', 'не распознается']], 'Отток датируется последним содержательным действием, а не датой удаления. Причина удаления хранится отдельно: самостоятельно, автоматически, администратором.')];
    R.notes = ['Открытый вопрос владельцев: можно ли после удаления хранить идентификатор, чтобы узнать вернувшегося человека. Это противоречит требованию удалить идентифицируемые данные — в продукте удаленный остается только обезличенной статистикой.'];
    R.how = '«Аккаунтов сейчас» — живая база; «за все время» — неубывающий счетчик. Стадии: спящие 15–30 дней без захода, предупрежденные 31–60, к удалению с 61-го.';
  },
  users(R, { q }) {
  ...
  data(R, { P }) {
    ...
    R.kpis = [kpi('Обновление аналитики', 'живое', { unit: '', sub: 'отчеты читают базу напрямую; ночного агрегата пока нет' }), kpi('Неизвестный источник', pct(unknownSrc, withMail), { unit: '%', sub: 'без UTM и кода приглашения', good: 'down' }), kpi('События без пользователя', orphan, { unit: `из ${total}`, good: 'down' }), kpi('Повторные события', dup, { unit: 'шт.', good: 'down' }), kpi('Тестовые аккаунты', staff, { unit: 'сотрудников', sub: 'исключаются из нормативов' }), kpi('Расходы без категории', unlinkedCosts, { unit: 'строк', good: 'down' }), nodata('Расхождение с биллингом', 'биллинг ИИ не подключен')];
    R.tables = [table('Что нужно для достоверности', ['Правило', 'Статус'], [['Единый внутренний идентификатор пользователя', 'есть'], ['Разделение тестовых и реальных аккаунтов', 'по списку сотрудников'], ['Единый перечень событий', 'есть (каталог в «Журнале событий»)'], ['Источник перехода внутри приложения', 'нет — событие без источника'], ['UTC, часовой пояс и локальная дата в событии', 'только UTC и день по МСК'], ['Версия приложения и экрана', 'нет'], ['Числитель, знаменатель и окно у процентов', 'показываются в каждом отчете'], ['Отметки релизов и кампаний на графиках', 'нет'], ['Ночной агрегат', 'нет — живой расчет'], ['Контроль повторных событий', 'проверка в этом отчете']])];
    R.how = 'Отчет проверяет саму аналитику: без этого проценты нельзя читать.';
  },
  content(R) {
  ...
  tickets(R) {
    ...
    R.how = 'Статусы: новое → в работе → ждем ответа пользователя → решено. Первый ответ — цель 30 минут. Ответ поддержки сразу виден человеку в приложении, в разделе «Чат».';
  },
  ...
  saved(R) { R.how = 'Сохраненные отчеты живут в этом браузере вместе с фильтрами.'; },
  ...
  materials(R) { R.how = 'Опубликованный «Вопрос дня» или «Аффирмация» с датой показа подменяет текст из файла в «Моем дне» у всех пользователей в этот день; без даты — действует каждый день, пока опубликован. Черновики и «на проверке» в приложение не попадают.'; },
  media(R) { R.how = 'Файл хранится на сервере рядом с базой и отдается по ссылке /app/uploads/…; ссылку можно вставить в материал или отдать разработчику. До 5 МБ: PNG, JPG, WebP, GIF, SVG, PDF.'; },

...
export function weekOf(d, pick) {
  let anchor = isDay(pick) ? pick : (dow(d) === 0 ? addDays(d, -1) : d);
...
export function createWeek({ db, open, seal, C, MOOD_RU, habitList, askesisList, track, nowISO, cleanText, setSource = () => '', mutate = null }) {
  ...
  /* Личные записи недели — один перечень и один способ чтения (аудит v98, F08): запись, благодарность, ответ на вопрос дня
     и мысль к материалу (с именем материала — чтобы вести к источнику). Итог недели (weekly) — отдельно, в reflectionOf */
  const PERSONAL_KINDS = ['', 'gratitude', 'answer', 'thought'];
  const thoughtName = (title) => { try { const m = JSON.parse(title); return m && typeof m === 'object' ? { name: m.name || '', question: m.question || '' } : {}; } catch { return {}; } };
  function recordsOf(uid, w) {
    return db.prepare(`SELECT id, day, kind, text, title FROM journal WHERE user_id = ? AND day BETWEEN ? AND ? AND kind IN (${PERSONAL_KINDS.map(() => '?').join(',')}) ORDER BY day, id`).all(uid, w.start, w.end, ...PERSONAL_KINDS)
      .map((r) => ({ id: r.id, day: r.day, kind: r.kind, text: open(r.text), ...(r.kind === 'thought' ? thoughtName(open(r.title || '')) : {}) })).filter((r) => r.text);
  }

  function moodsOf(uid, w) {
    const byDay = Moods.byDay(uid, w.start, w.end);
  ...
  function savedOf(rows) {
    ...
    return pick.sort((a, b) => a.day.localeCompare(b.day) || a.id - b.id).map((r) => ({ day: r.day, kind: r.kind, text: trim(r.text), ...(r.kind === 'thought' ? { name: r.name || '', question: r.question || '' } : {}) }));
  }
  ...
  /* Календарная неделя пн–вс (не «последние 7 дней» отчета по настроениям — тот в server.mjs weekSummary).
     Моменты недели — все, что человек оставил: тексты (с мыслями к материалам), дни с настроением, фото дня, заметки к аскезе;
     mode выбирает только вступление, разделы показываются по наличию данных (F08) */
  function state(u, d, pick) {
    const w = weekOf(d, pick);
    const rows = recordsOf(u.id, w);
    const moods = moodsOf(u.id, w);
    const photos = db.prepare('SELECT COUNT(*) c FROM day_photos WHERE user_id = ? AND day BETWEEN ? AND ?').get(u.id, w.start, w.end).c;
    const notes = db.prepare('SELECT n.note FROM askesis_days n JOIN askesis a ON a.id = n.askesis_id WHERE a.user_id = ? AND n.day BETWEEN ? AND ?').all(u.id, w.start, w.end).filter((n) => open(n.note || '')).length;
    const moments = rows.length + moods.count + photos + notes;
    ...
    return {
      ...
      mode, moments, counts: { texts: rows.length, moodDays: moods.count, photos, notes }, text: mode === 'empty' ? texts('пусто')[0] || '' : mode === 'few' ? few : '',
      moods, saved: savedOf(rows), echoes: echoesOf(u.id, w, d, rows), rhythm: rhythmOf(u.id, w, d), reflection: reflectionOf(u.id, w),
    };
  }
  /* Отметка «отозвалось»: одна на день, пустая — снимает (то же делает day.save с полем echo — вечером, одним запросом) */
  function echo(u, d, b) {

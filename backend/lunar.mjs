/* Лунный день: номер и границы для точки на Земле.
   ...
   Астрономия по Меесу (укороченные ряды): Солнце ±0.01°, Луна ±0.05° —
...
// ряды Мееса (гл. 47): аргументы D, M, M', F и коэффициенты в 1e-6 градуса / метрах
...
// высота центра Луны над горизонтом минус порог восхода (учет параллакса и рефракции)
...
export function newMoonBefore(ms) {
  ...
  let e = elong(jd);
  // идем назад, пока элонгация не «перепрыгнет» через 0 (стала отрицательной)
  for (let i = 0; i < 200; i++) {
...
/* Освещенность диска и доля цикла на момент времени */
export const MOON_NAMES = ['Новолуние', 'Растущий серп', 'Первая четверть', 'Растущая Луна', 'Полнолуние', 'Убывающая Луна', 'Последняя четверть', 'Убывающий серп'];

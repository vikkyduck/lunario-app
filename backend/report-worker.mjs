/* Отчёты кабинета считаются в отдельном потоке (worker_threads).
   SQLite в node синхронна: один отчёт активации на большой базе задерживал все запросы приложения
   примерно на секунду. Здесь своё соединение с той же базой (WAL: читатель не мешает писателю),

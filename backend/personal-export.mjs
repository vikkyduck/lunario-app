...
export function personalExport(db,u,open){
  ...
  const habits=rows('SELECT id,title,created_at,archived,rule,rule_text FROM habits WHERE user_id=? ORDER BY id').map(r=>({...r,title:open(r.title),rule_text:open(r.rule_text||''),marks:db.prepare('SELECT day FROM habit_marks WHERE habit_id=? ORDER BY day').all(r.id).map(x=>x.day),awards:db.prepare('SELECT days,ts FROM habit_awards WHERE habit_id=? ORDER BY days').all(r.id)}));
  ...
  return {version:2,exportedAt:new Date().toISOString(),preferences:preferences(u.preferences),profile:{name:u.name,birth:u.birth,birthTime:u.birth_time,city:u.city,email:u.email,photo:u.photo},journal,wishes,habits,askesis,

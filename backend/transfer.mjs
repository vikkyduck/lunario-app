/* Перенос записей гостя в аккаунт, в который человек только что вошёл.
   ...
   только кнопка «перенести», и её можно получить снова, войдя ещё раз.

   Правило конфликта. У части таблиц ключ — человек и день (настроение дня, установка дня, счётчик раскладов).
   Если в аккаунте на этот день уже что-то есть, побеждает то, что в аккаунте: переносить поверх — значит
   затирать более поздний осознанный выбор. Что не поместилось, показываем в отчёте, а не умалчиваем. */
...
export function transferGuestRecords(db, guestId, accountId) {
  ...
  try {
    ...
    for (const t of MOVED_TABLES) {
      ...
      if (!before) continue;
      /* OR IGNORE: строка, у которой в аккаунте уже занят тот же день, остаётся у гостя и удаляется ниже */
    ...
    /* Досье гостя не нужно: у аккаунта своё, и оно пересоберётся из переехавших записей.

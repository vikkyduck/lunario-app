/* Очередь тяжелых заданий в отдельном потоке (report-worker.mjs) для основного процесса (ревью v114, F09, F10):
   отчеты и дашборд кабинета (kind overview/report) и полная выгрузка личных данных (kind export — JSON или PDF).
   SQLite в node синхронна: отчет на большой базе или выгрузка года записей с PDF в основном потоке останавливали бы
   /api/health и все запросы людей на сотни миллисекунд. Поток поднимается при первом обращении и переживает ошибки задания.
...
/* maxByKind — свой предел на вид задания (выгрузок в очереди не больше 4, F09); одинаковые задания (в том числе две выгрузки одного
   человека подряд) объединяются в одно — второй запрос получает тот же результат, а не второй расчет */
export function createJobRunner({ dataDir, inline = {}, log = console.log, workerUrl = new URL('./report-worker.mjs', import.meta.url), timeoutMs = 60000, maxByKind = {} }) {
  ...
  function call(kind, args) {
    ...
    if (maxByKind[kind] && [...inflight.values(), ...pending].filter((j) => j.kind === kind).length >= maxByKind[kind]) return Promise.reject(busyError());
    ...
    const p = new Promise((resolve, reject) => {
      ...
      if (e.why === 'worker_unavailable' && !everSpawned && inline[kind]) { log('Отчеты: поток недоступен — считаю в основном потоке один раз'); return inline[kind](...args); }
      throw e;
    });
    byKey.set(key, p);
  ...
  return {
    ...
    /* полная выгрузка личных данных (F09): JSON — объект, PDF — байты (Buffer); собирается в потоке в согласованном снимке чтения */
    export: (userId, format) => call('export', [userId, format]).then((r) => (format === 'pdf' ? Buffer.from(r.buffer, r.byteOffset, r.byteLength) : r)),

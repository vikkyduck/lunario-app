/* Лунарио — «Я помню»: приложение вспоминает человека одной фразой, а не считает его.
   ...
   Источник — те же таблицы, что у экранов и полок (entries, journal, moods, habits, askesis, users): правилам нужны точные даты,
   поэтому читаются таблицы, а не снимок полок.

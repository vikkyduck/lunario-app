...
import { featureTitle as T } from './features.mjs';
...
export const EVENTS = [
  ...
  E('card_open', 'Открыл карту дня', 'server', { core: true, section: 'Сегодня', title: T('card') }),
  E('dayrune_open', 'Открыл руну дня', 'server', { core: true, section: 'Сегодня', title: T('dayrune') }),
  ...
  E('forecast_view', 'Открыл прогноз дня', 'client', { core: true, section: 'Сегодня', title: T('day') }),
  E('answer_add', 'Ответил на вопрос дня', 'server', { core: true, section: 'Сегодня', title: T('tone') }),
  E('thought_save', 'Записал мысль к карте, руне или раскладу', 'server', { core: true, section: 'Сегодня', title: 'Мысль к материалу' }),
  E('habit_add', 'Добавил привычку', 'server', { core: true, section: 'Сегодня', title: T('habits') }),
  E('habit_mark', 'Отметил привычку', 'server', { core: true }),
  E('askesis_start', 'Взял аскезу', 'server', { core: true, section: 'Сегодня', title: T('askesis') }),
  ...
  E('lunar_view', 'Открыл лунный день', 'client', { core: true, section: 'Луна и небо', title: T('lunar') }),
  ...
  E('worry_pick', 'Разобрал вопрос', 'client', { core: true, section: 'Свериться с собой', title: T('worry') }),
  ...
  E('gratitude_add', 'Записал благодарность', 'server', { core: true, section: 'Дневник', title: T('gratitude') }),
  E('wish_add', 'Добавил желание', 'server', { core: true, section: 'Дневник', title: T('wishes') }),
  E('wish_photo', 'Добавил фото к желанию', 'server'),
  E('moodreport_view', 'Открыл отчет по настроениям', 'client', { core: true, section: 'Дневник', title: T('hmood') }),
  E('week_view', 'Открыл «Мою неделю»', 'server', { core: true, section: 'Дневник', title: T('week') }),
  E('week_echo', 'Отметил, отозвалось ли утро', 'server'),
  E('week_reflect', 'Записал рефлексию недели', 'server', { core: true }),
  E('natal_view', 'Открыл натальную карту', 'server', { core: true, section: 'Обо мне', title: T('natal') }),
  E('compat_calc', 'Посчитал совместимость', 'server', { core: true, section: 'Обо мне', title: T('compat') }),

/* Надежная запись: повтор одного действия не создает второй записи.

   Зачем. Ответ может потеряться по дороге — сеть оборвалась, телефон уснул, вкладку закрыли. Человек нажимает
   «Сохранить» еще раз, и без защиты в дневнике оказываются две одинаковые записи. Поэтому клиент придумывает
   ...
   · Запись и квитанция должны попасть в базу вместе. Если записать их раздельно, отказ между ними оставит
     либо запись без подтверждения (повтор задвоит), либо подтверждение без записи (текст пропадет).
   ...
   · Ошибку базы нельзя показывать человеку. «database is locked» — это «попробуйте еще раз», а не поломка;
...
export function saveJournalOperation(db, principal, body, deps) {
  ...
  /* Отпечаток тела — по нормализованному виду: тот же текст после повтора дает тот же отпечаток. */
  ...
  /* Аналитика и серия — уже после COMMIT: они не должны отменять сохраненную запись, если сами не заладятся,
...
/* Уборка старых квитанций: их держат ради повторов, а не как историю. Зовется редко, из фоновой уборки. */

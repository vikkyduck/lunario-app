...
const ARCANA_FALLBACK = [
  ['0 · Шут', 'Начало без гарантий: вы знаете меньше, чем хотелось бы, и это нормально.', 'Разрешите себе сделать шаг, не просчитав все до конца.'],
  ...
  ['VI · Влюбленные', 'Выбор, который нельзя сделать наполовину.', 'Назовите вслух, чего вы хотите на самом деле.'],
  ...
  ['X · Колесо Фортуны', 'Цикл меняется, и не все зависит от вас.', 'Не удерживайте то, что уходит само.'],
  ['XI · Справедливость', 'Все возвращается по факту, а не по намерению.', 'Проверьте, где вы обещали больше, чем можете.'],
  ...
  ['XVIII · Луна', 'Много тумана: страх выдает себя за интуицию.', 'Отложите важное решение до завтра.'],
  ...
  ['XXI · Мир', 'Круг замкнулся: этап завершен по-настоящему.', 'Отметьте то, что сделали, прежде чем брать новое.'],
];
...
const DAY_TONES_FALLBACK = [
  ...
  ['День разговоров', 'Слова сегодня весят больше обычного: важный разговор пройдет легче, чем вы ждете.', {work:70,love:80,health:60,money:50}],
  ['День на восстановление', 'Тело просит паузы. Все, что можно перенести, — переносите без вины.', {work:45,love:60,health:85,money:40}],
...
const RUNES_FALLBACK = [
  ...
  ['Ансуз', 'весть', 'M11 5v22M11 8l10 6M11 15l10 6', 'Ответ придет через слова: разговор, письмо, случайную фразу. Слушайте.'],
  ...
  ['Гебо', 'обмен', 'M9 6l14 20M23 6 9 26', 'Здесь все на взаимности: не тяните в одиночку и не ждите молча.'],
  ['Вуньо', 'радость', 'M11 5v22M11 5l9 6-9 6', 'Хороший знак: радости тут больше, чем тревоги. Позвольте себе ее.'],
  ['Альгиз', 'защита', 'M16 27V9M8 5l8 8 8-8', 'Вы защищеннее, чем чувствуете. Держитесь своих границ — они и есть опора.'],
  ['Наутиз', 'нужда', 'M10 27 22 5M11 13l10 6', 'Есть ограничение, которое придется признать. Через него — короче, чем в обход.'],
  ['Иса', 'пауза', 'M16 5v22', 'Сейчас ничего не двигается — и не должно. Подождите, лед сойдет сам.'],
  ['Йера', 'урожай', 'M20 5 12 13M12 19l8 8M12 13l-4 6', 'Собираете посеянное раньше. Результат придет в свой срок, не раньше.'],
  ['Беркана', 'рост', 'M11 5v22M11 7l9 5-9 5M11 17l9 5-9 5', 'Начинается тихий рост. Не дергайте росток, чтобы проверить корни.'],
  ['Турисаз', 'преграда', 'M11 5v22M11 9l9 6-9 6', 'Впереди то, что придется выдержать. Не лезьте напролом — обойдите или переждите.'],
  ...
  ['Перт', 'тайна', 'M20 5h-6l6 11-6 11h6', 'Что-то еще скрыто от вас. Не решайте окончательно — картина не полная.'],
  ['Соулу', 'победа', 'M21 5h-8l6 11-8 11', 'Энергия на вашей стороне. Действуйте, пока идет эта волна.'],
  ['Тейваз', 'решимость', 'M16 27V11M8 11l8-6 8 6', 'Нужен прямой выбор и честность — прежде всего с собой. Назовите вещи именами.'],
  ['Эваз', 'движение', 'M10 27V9l6 6 6-6v18', 'Перемена уже началась, и она к лучшему. Не тормозите ее сомнением.'],
  ['Манназ', 'человек', 'M10 27V5l6 9 6-9v22M10 9l12 12', 'Ответ придет через людей. Спросите того, чье мнение цените.'],
  ['Лагуз', 'поток', 'M12 5v22l8-8', 'Доверьтесь течению: сейчас чутье точнее расчета.'],
  ['Ингуз', 'завершение', 'M10 9l6 7 6-7-6 7-6 7 6-7 6 7', 'Цикл закрывается. Доведите начатое — и отпустите.'],
  ['Одал', 'свое', 'M16 27V15M16 15 9 8l7-3 7 3-7 7', 'Речь о вашем: о доме, роде, о том, что принадлежит только вам. Берегите это.'],
  ['Дагаз', 'рассвет', 'M10 5v22l12-22v22z', 'Перелом близко — и он светлый. Худшее позади, даже если еще не видно.'],
];
...
const ASKESIS_FALLBACK = ['Без сахара', 'Без кофе после обеда', 'Без соцсетей после 21:00', 'Без жалоб', 'Без сплетен и обсуждения других', 'Без алкоголя',
  'Без покупок не по списку', 'Подъем до 7:00', '10 минут тишины каждый день', 'Прогулка каждый день', 'Без сериалов в будни', 'Без телефона за едой'];
...
const REMINDER_TEXTS_FALLBACK = {
  ...
  evening: ['Среди шума дня остался ваш голос', 'Дайте ему слово в Лунарио'],
  'evening-2': ['Останьтесь с собой на минуту', 'Без спешки, без фильтров'],
  'evening-3': ['Минута для себя перед сном', 'Только вы и ваши мысли'],
  'evening-4': ['Этот день заслуживает красивой точки', 'Давайте поставим ее вместе'],
...
const NASTROY_FALLBACK = [
  ['ровный', 'Спокойствие — это тоже результат.', 'Что в моем дне сегодня работает само, без моих усилий?'],
  ['разговоры', 'Мне можно говорить о том, что для меня важно.', 'Что я хочу сказать вслух сегодня?'],
  ['восстановление', 'Моя усталость — не слабость, а сигнал.', 'Что мое тело просит у меня сегодня?'],
  ...
  ['выдох', 'Я разрешаю себе идти в своем темпе — этого достаточно.', 'Что я могу сегодня отпустить, не дожидаясь разрешения?'],
];
const THEME_KEY = /^[а-яе][а-яе-]{1,24}$/;

/* ── Нумерология ── */
const NUM_DESTINY_FALLBACK = {
  1: ['Число самостоятельных решений', 'Вам легче начать самой, чем ждать разрешения. Лучшие периоды — когда вы ведете, а не догоняете.'],
  ...
  4: ['Число опоры', 'Вы строите надежно и не любите спешки. Ваша устойчивость — то, за чем к вам приходят.'],
...
const NUM_YEAR_FALLBACK = {
  ...
  9: 'Год завершения большого цикла: завершать, отдавать, расчищать место. Большое новое придет в следующем.',
};
...
const NUM_DAY_FALLBACK = {
  1:'День самостоятельных шагов.',2:'День союзов и договоренностей.',3:'День слов и общения.',
...
const TOOLS_FALLBACK = [
  ['journal', 'history', 'Пара строк', 'Что хочется оставить от этого дня — своими словами', 'нет'],
  ['answer', 'history', 'Вопрос дня', 'Вечером — ответ на утренний вопрос дня', 'нет'],
  ...
  ['photo', 'history', 'Фото дня', 'Один снимок на день — рядом с записью, в строке дня и в открытом дне', 'нет'],
...
const AFFIRMATIONS_FALLBACK = [
  'Я разрешаю себе идти в своем темпе — этого достаточно.',
...
const DAY_QUESTIONS_FALLBACK = [
  ...
  'Что было теплым за последние сутки?',
...
const YN_RIDERS_FALLBACK = {
  work:  ['Про работу: сейчас на вашей стороне и время, и репутация — действуйте прямо.',
          'Про работу: решение выглядит верным, но информации у вас меньше, чем кажется. Соберите ее.',
          'Про работу: начните с малого — разговор, письмо, пробный день. Большое созреет из него.'],
  money: ['Про деньги: расчет сходится. Если сумма не последняя — можно.',
          'Про деньги: не сейчас. Эта трата из тревоги, а не из расчета — вернитесь через две недели.',
  ...
  love:  ['Про отношения: скажите прямо то, что держите в себе. Именно это сдвинет ситуацию.',
          'Про отношения: не сегодня. Разговор на этой волне уйдет в обиду, а не в близость.',
          'Про отношения: начните не с выяснения, а с простого теплого шага навстречу.'],
  health:['Про состояние: тело дает добро. Только не превращайте это в очередной подвиг.',
  ...
  study: ['Про учебу: начинайте. У вас есть и время, и запал — этого достаточно.',
          'Про учебу: не сейчас. Вы берете это из чувства долга и бросите на третьей неделе.',
          'Про учебу: да, но малым форматом. Одна короткая программа вместо большой.'],

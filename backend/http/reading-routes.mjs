...
import { ELEMENT_OF, MODALITY_OF } from '../natal-texts.mjs';
/* Совместимость по знакам Солнца двух дат (повторный аудит v112, R16): каждое кольцо — из одного названного правила, и это
   правило выводится рядом с числом. Числа — условная шкала для разговора, не измерение отношений: так и сказано на экране.
   Стихии: своя — 90, дружественные (огонь+воздух, земля+вода) — 80, остальные — 60. Кресты: кардинальный+мутабельный — 85,
   фиксированный+мутабельный — 75, кардинальный+фиксированный — 70, одинаковые — 65 (двое ведущих или двое держащих).
   Угол между знаками: тригон (4 знака) — 90, секстиль (2) — 85, свой знак — 80, оппозиция (6) — 70, полусекстиль (1) — 65,
   квадрат (3) — 60, квинконс (5) — 55. Притяжение: противоположные стихии (огонь↔вода, воздух↔земля) — 85, дружественные — 75, своя — 70 */
const SIGN_ORDER = ['Овен', 'Телец', 'Близнецы', 'Рак', 'Лев', 'Дева', 'Весы', 'Скорпион', 'Стрелец', 'Козерог', 'Водолей', 'Рыбы'];
const FRIENDS = { 'огонь': 'воздух', 'воздух': 'огонь', 'земля': 'вода', 'вода': 'земля' };
const OPPOSITE = { 'огонь': 'вода', 'вода': 'огонь', 'воздух': 'земля', 'земля': 'воздух' };
const ANGLE = { 0: [80, 'один знак — похожий темп'], 1: [65, 'соседние знаки — разный язык'], 2: [85, 'секстиль — легкое сотрудничество'], 3: [60, 'квадрат — трение и рост'], 4: [90, 'тригон — естественное согласие'], 5: [55, 'квинконс — надо подстраиваться'], 6: [70, 'оппозиция — притяжение и спор'] };
export function compatRings(a, b) {
  const ea = ELEMENT_OF[a] || '', eb = ELEMENT_OF[b] || '', ma = MODALITY_OF[a] || '', mb = MODALITY_OF[b] || '';
  const emo = ea === eb ? [90, `одна стихия — ${ea}`] : FRIENDS[ea] === eb ? [80, `дружественные стихии — ${ea} и ${eb}`] : [60, `разные стихии — ${ea} и ${eb}`];
  const pair = [ma, mb].sort().join('+');
  const talk = ma === mb ? [65, `оба знака ${ma}е — двое ведущих или двое держащих`] : pair === 'кардинальный+мутабельный' ? [85, 'кардинальный и мутабельный кресты дополняют друг друга'] : pair === 'мутабельный+фиксированный' ? [75, 'фиксированный и мутабельный — один держит, другой подстраивается'] : [70, 'кардинальный и фиксированный — двое со своим планом'];
  const k = Math.abs(SIGN_ORDER.indexOf(a) - SIGN_ORDER.indexOf(b)), dist = Math.min(k, 12 - k), ang = ANGLE[dist] || [60, ''];
  const pas = OPPOSITE[ea] === eb ? [85, `противоположные стихии — ${ea} и ${eb}`] : ea === eb ? [70, `одна стихия — ${ea}`] : [75, `дружественные стихии — ${ea} и ${eb}`];
  return [['Эмоции', emo[0], 'по стихиям знаков: ' + emo[1]], ['Общение', talk[0], 'по крестам знаков: ' + talk[1]], ['Быт', ang[0], 'по углу между знаками: ' + ang[1]], ['Страсть', pas[0], 'по притяжению стихий: ' + pas[1]]];
}
...
export function createReadingRoutes({ compatSave, natalMeanings, C, cardOfDay, cardPublic, clean, DAILY_WRITES, dayNum, db, destinyNum, drawDistinct, hash32, ISO_DAY, json, markOpened, Morning, natalFor, nowISO, numFormula, parseData, personalYearAt, readBody, runePublic, seal, signOf, topicOf, touchStreak, track }) {
  return async function readingRoutes({ p, req, res, url, u, d }) {
    ...
    if (p === '/api/compat' && req.method === 'POST') {
      ...
      const rings = compatRings(a.name, o.name);   /* каждое кольцо — из названного правила по знакам Солнца (R16) */
      track(u, 'compat_calc', '');

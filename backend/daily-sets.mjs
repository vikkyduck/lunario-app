...
export function initDailySets(db, legacySets = []) {
  ...
  for (const col of ['text_key', 'text', 'question']) {
...
export function dailySet(db, user, day, sets) {
  ...
  if (!row?.text) {
    ...
    db.prepare(`INSERT INTO daily_sets(user_id,day,idx,text_key,text,question) VALUES(?,?,?,?,?,?)
      ON CONFLICT(user_id,day) DO UPDATE SET idx=excluded.idx,text_key=excluded.text_key,text=excluded.text,question=excluded.question WHERE daily_sets.text=''`)
      .run(user.id, day, chosen.idx, chosen.key, chosen.s[1], chosen.s[2]);

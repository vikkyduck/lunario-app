...
/* Копия базы — это все личные записи. Раньше она лежала в одной папке с ключом шифрования: у того, кто получил
   ...
   Это не мешает главному: живой сервер по-прежнему читает записи своим ключом и отдаёт их досье и разборам —
   шифрование копий к работе приложения отношения не имеет.

   BACKUP_SECRET задаётся в окружении сервиса (systemd EnvironmentFile), в папке копий его нет.

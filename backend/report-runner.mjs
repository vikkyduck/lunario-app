/* Обертка над потоком отчетов (report-worker.mjs) для основного процесса.
   ...
   Поток поднимается при первом обращении и переживает ошибки отчета; если он упал или не отвечает,
   отчет один раз считается в основном потоке (inline) — кабинет не остается без данных. */
...
export function createReportRunner({ dataDir, inline, log = console.log }) {
  ...
  function call(kind, args) {
    return new Promise((resolve, reject) => {
      ...
      if (e.fromReport || e.message === 'report_timeout') throw e;   // ошибка самого отчета или он слишком долгий — не пересчитываем в основном потоке
      log(`Отчеты: поток недоступен (${e.message}) — считаю в основном потоке`);

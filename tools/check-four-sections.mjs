...
export async function checkFourSections({browser,base,owner}){
  ...
  try{
   ...
   для человека это тот же текст, и проверка не должна зависеть от вида пробела */
    ...
    assert.deepEqual(await page.locator('#morning-chips .chip').allTextContents(),['Карта дня','Руна дня','Планеты','Прогноз дня','Луна','Вопрос дня']);
    assert.ok(await page.locator('#morning-picker').isVisible()&&!(await page.locator('#morning-summary').isVisible()),'the question is open until answered');
    ...
    assert.ok(await page.locator('#morning-chips [data-a0=card]').evaluate(el=>document.activeElement===el&&el.getAttribute('aria-pressed')==='true'),'focus stays on the pressed chip');
    await page.locator('#morning-done').click();assert.ok(await page.locator('#morning-summary').isVisible()&&(await page.locator('#morning-summary-list').innerText()).includes('Карта дня'),'after the answer the question folds into one line');
    await page.locator('#morning-summary').click();assert.ok(await page.locator('#morning-picker').isVisible(),'«Изменить» opens the question again');

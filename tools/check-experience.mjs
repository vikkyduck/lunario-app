...
export async function checkExperience({browser,base,owner}){
  ...
  try{
    ...
    await page.locator('#h-acct').click();await page.locator('#w-appearance .theme-option.dark').click();await page.waitForFunction(()=>document.documentElement.dataset.theme==='dark');await close();   /* тема — кружком справа вверху, три варианта */

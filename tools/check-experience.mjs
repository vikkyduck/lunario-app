...
export async function checkExperience({browser,base,owner}){
  ...
  try{
    ...
    await page.locator('#h-acct').click();await page.locator('#theme-toggle').click();await page.waitForFunction(()=>document.documentElement.dataset.theme==='dark');   /* тема — значком рядом с фото в профиле */

...
export async function checkRestoration({browser,base,owner}){
  ...
  try{
    ...
    const prompt=catalogue.worries[0];await page.locator('#hub-questions button').first().click();
    assert.equal(await page.locator('#hub-q').inputValue(),prompt);
    const edited=prompt+' Что мне важно понять?';await page.locator('#hub-q').fill(edited);
    await page.locator('#hub-chips button').nth(1).click();assert.equal(await page.locator('#hub-q').inputValue(),edited);
